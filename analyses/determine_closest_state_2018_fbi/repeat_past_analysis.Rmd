...
nestedFBIData <- FBIData %>%
  ...
  select(state, year, robbery, property_crime, burglary, larceny_theft, motor_vehicle_theft, estimated_population) %>%
...
for (i in seq_along(nestedFBIData$state)) { 
  frames[i] <- nest(data = everything(), rbind(nestedFBIData[[i, "other_states"]], nestedFBIData[nestedFBIData$state == "Massachusetts", ][[2]][[1]]))
}
